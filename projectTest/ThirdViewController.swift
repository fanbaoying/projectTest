...
class ThirdViewController: UIViewController {
    ...
    fileprivate lazy var leftTableView: UITableView = {
        let myTableView = UITableView.init(frame: CGRect(x: 0, y: 0, width: SCR_W * 0.25, height: SCR_H-49), style: UITableViewStyle.plain)
        myTableView.delegate = self
        myTableView.dataSource = self
        return myTableView
    }()
    fileprivate lazy var rightTableView: UITableView = {
        let tableView = UITableView.init(frame: CGRect(x: SCR_W * 0.3, y: 64, width: SCR_W * 0.7, height: SCR_H-103), style: UITableViewStyle.plain)
